...
# Disable InsecureRequestWarning (usually for urllib3/requests, not strictly needed for aiohttp but good for context)
...
def to_superscript(number_str):
    return ''.join(SUPERSCRIPT_MAP[d] for d in number_str)

def generate_random_name():
    random_digits = ''.join(random.choice(string.digits) for _ in range(3))
    superscript_digits = to_superscript(random_digits)
    name = f"T.Quí乂Tmr{superscript_digits}"
    return name

def generate_custom_password():
    random_digits = ''.join(random.choice(string.digits) for _ in range(9))
    return f"Tmr{random_digits}_VsTeam"

async def create_acc(region, session):
    ...
    try:
        async with session.post(url, headers=headers, data=data) as response:
            ...
            uid = res_json['uid']
            return await token(uid, password, region, session)
    except Exception as e:
        # Recursively call create_acc on failure
        return await create_acc(region, session)

async def token(uid, password, region, session):
    ...
    async with session.post(url, headers=headers, data=body) as response:
        ...
        return await Major_Regsiter(access_token, open_id, field, uid, password, region, session)
...
async def Major_Regsiter(access_token, open_id, field, uid, password, region, session):
    ...
    async with session.post(url, headers=headers, data=body) as response:
        return await login(uid, password, access_token, open_id, (await response.read()).hex(), response.status, name, region, session)
...
async def chooseregion(data_bytes, jwt_token, session):
...
async def login(uid, password, access_token, open_id, response, status_code, name, region, session):
    ...
    async with session.post(URL, headers=headers, data=Final_Payload) as RESPONSE:
        if RESPONSE.status == 200:
            text = await RESPONSE.text()
            if len(text) < 10:
                return False
            if lang.lower() not in ["ar", "en"]:
                content_hex = (await RESPONSE.read()).hex()
                json_result = get_available_room(content_hex)
                ...
                r = await chooseregion(fields, BASE64_TOKEN, session)

                if r == 200:
                    return await login_server(uid, password, access_token, open_id, response, status_code, name, region, session)
            else:
                BASE64_TOKEN = text[text.find("eyJhbGciOiJIUzI1NiIsInN2ciI6IjEiLCJ0eXAiOiJKV1QifQ"):-1]
            second_dot_index = BASE64_TOKEN.find(".", BASE64_TOKEN.find(".") + 1)
            await asyncio.sleep(0.2)
            BASE64_TOKEN = BASE64_TOKEN[:second_dot_index+44]
            dat = await GET_PAYLOAD_BY_DATA(BASE64_TOKEN, access_token, 1, response, status_code, name, uid, password, region, session)
            return dat

async def login_server(uid, password, access_token, open_id, response, status_code, name, region, session):
    lang = get_region(region)
    lang_b = lang.encode("ascii")
...
async def GET_PAYLOAD_BY_DATA(JWT_TOKEN, NEW_ACCESS_TOKEN, date, response, status_code, name, uid, password, region, session):
    try:
        ...
        print(JWT_TOKEN)
        data = await GET_LOGIN_DATA(JWT_TOKEN, PAYLOAD, region, session)
        print(data)
        print(uid, password)
...
async def GET_LOGIN_DATA(JWT_TOKEN, PAYLOAD, region, session):
    if region.lower() == "me":
...
async def worker(region, session, file_lock):
    try:
        r = await create_acc(region, session)
        # If r is False (failed somewhere), return
        if not r: return
        ...
        if status_code == 200:
            async with file_lock:
                with open('acckhach.txt', 'a') as file:
...
async def main():
    ...
    # Limit concurrent connections to avoid network flood or bans
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        tasks = []
        for _ in range(500):
            # Passing semaphore control to worker if needed, 
            # but limiting via gathering batches or just relying on session limits is often easier.
            # Here we just fire all tasks, but the ClientSession limit=100 handles open connections.
            tasks.append(worker(region, session, file_lock))
